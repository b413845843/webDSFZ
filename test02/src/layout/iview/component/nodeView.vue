...
<template>
    <div>
        <h3>{{title}}</h3>
        <div v-for="{title,content,datas},index in items" class="leaf">
            <h4 v-if="datas">{{title}}</h4>
            <div v-if="datas" class="leaf"> 
                <p v-for="data in datas"><span class="leafTitle">{{data.title}}</span>  : <span class="leafContent">{{data.content}}</span></p>
            </div>
...
<script>
// import {Node} from  "@/tools/parseJsonToNode";
export default {
    name:'nodeView',
    props:{
        node:Object
